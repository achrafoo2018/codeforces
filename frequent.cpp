...
public:
	...
	Tree_Type query(int l, int r){
 	...
 	Tree_Type query(int l, int r, int x, int lx, int rx){
		if(lx >= r || l >= rx) return MP(0, 0);
		if(lx >= l && rx <= r) return tree[x];
		int m = (lx + rx) / 2;
		pii left = query(l, r, 2*x+1, lx, m);
		pii right = query(l, r, 2*x+2, m, rx);
		return merge(left, right);
	}

	Tree_Type merge(Tree_Type left, Tree_Type right){
		pii ans = left;
