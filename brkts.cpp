...
public:
	...
	void update(int i, int x, int lx, int rx){
		if(rx - lx == 1){
			tree[x].opening = !tree[x].opening;
			tree[x].closing = !tree[x].closing;
			return;
		}
		int m = (lx + rx) / 2;
		if(i < m){
	...
	Tree_Type merge(int x){
		...
		Tree_Type ret = node();
		ret.opening = left.opening + right.opening - left.closing;
		ret.closing = left.closing + right.closing - left.opening;
		return left + right;
	}
};
