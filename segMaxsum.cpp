...
struct node{
	ll ans, pref, suff, s;
	node(ll ans, ll pref, ll suff, ll s) : ans(ans), pref(pref), suff(suff), s(s){};
};
...
public:
	...
	void init(int n){
		size = 1;
		while(size < n) size *= 2;
		tree.assign(size * 2, node(0, 0, 0, 0));
	}
	...
	Tree_Type merge(int x){
		...
		ll pref = max(left.pref, left.s + right.pref);
...
void solve(){
	...
	For(i, n)
		cin >> arr[i];
	segment_tree<ll, node> st;
	st.build(arr);
	For(i, m){
		int x, v; cin >> x >> v;
		st.update(x, v);
		cout << st.tree[0].ans << endl;
	}
}
